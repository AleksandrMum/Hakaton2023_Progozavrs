...
/**
	* Sets the protocol to RSsend, from a list of predefined protocols
	*/
...
/**
	* Sets the protocol to RSsend with pulse length in microseconds.
	*/
 ...
 *
 * @param nTransmitterPin	 Arduino Pin to which the RSsender is connected to
 */
...
/**
 * Returns a char[13], representing the code word to be RSsend.
 *
 */
 ...
 */
void RSsendTriState(RCSWITCH_t * RCSwitch, const char* sCodeWord) {
	// turn the tristate code word into the corresponding bit pattern, then RSsend it
 ...
 */
void RSsend(RCSWITCH_t * RCSwitch, unsigned long code, unsigned int length) {
	...
	// Disable transmit after RSsending (i.e., for inverted protocols)
...
bool receiveProtocol(RCSWITCH_t * RCSwitch, const int p, unsigned int changeCount) {
	...
	if (changeCount > 7) {		// ignore very short transmissions: no device RSsends them, so this must be noise
...
void handleInterrupt(void* arg)
{
	...
	if (duration > RCSwitch->nSeparationLimit) {
		...
		if (diff(duration, RCSwitch->timings[0]) < 200) {
			...
			// here that a RSsender will RSsend the signal multiple times,
